...
if(isset($_GET['key']) || isset($_POST['key']) || (isset($_POST['options']) && isset($_POST['options']['type']) && $_POST['options']['type'] == 'pass_reset')) {
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if(isset($_POST['method']) && $_POST['method'] == 'createOrUpdate') {
			echo json_encode($requestHandler->execute($_POST['type'], $_POST['method'], $_POST['query']));
		} else {
			json_encode($requestHandler->update($_POST['query'], $_POST['options']));
		}
	} else {
		$type = $_GET['type'];
		$method = $_GET['method'];
